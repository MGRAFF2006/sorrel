...
| `sorrel-cli` | Done / merged | **P0 complete**: persistent `init`/`status`/`change create`/`diff`/`log` on git-dep engine | PR #9 merged; root points to `sorrel-cli/main` at `81058ae`; full local VCS demo (init->edit->status->change->diff->log) persists on disk; line-level diff; snapshot-DAG log; `cargo test --workspace` all pass (json_output 24, lib 8), fmt + clippy clean. See `sorrel-cli/DEMO.md`. |
