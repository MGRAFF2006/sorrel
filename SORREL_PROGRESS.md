...
| `sorrel-cli` | Done / merged | Real persistent `init`/`status` (P0-2/3) on git-dep engine | PR #7 merged; root points to `sorrel-cli/main` at `a40b34f`; `init` writes `.sorrel` object store + manifest + atomic HEAD, `status` reads them; persistence-across-processes test; `cargo test --workspace` all pass, fmt + clippy clean. |
