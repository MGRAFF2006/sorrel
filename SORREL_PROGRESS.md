...
| `sorrel-core` | Done / merged | VCS engine (store/snapshot/change/lanes) + native policy/authority + `materialize_snapshot_excluding` + perf benches; `cli_policy` compat removed | Root points to `sorrel-core/main` at `047b13b`; `cargo test`/`cargo bench` pass; fmt + clippy clean. |
| `sorrel-cli` | Done / merged | **All commands real & persistent**; now **lib+bin** owning `cli_policy`/`cli_runner` (compat retired from engine crates) | Root points to `sorrel-cli/main` at `052ebde`; engine-level `.sorrel` exclusion; no `mocked:true` outputs; `.sorrel/{lanes,grants,secrets,slices}` registries; only `sorrel-core` git dep (dropped `sorrel-runners`); `cargo test --workspace` all pass (lib 18, json_output 25, conformance 2, workflow 7); fmt + clippy clean. See `sorrel-cli/DEMO.md`. |
...
| `sorrel-sdk-rust` | Not started | Rust SDK | Start after core APIs settle. AGENTS/README fleshed out. |
...
| 2026-06-26 | Cleanup D DONE. `sorrel-core` `41a27e7`: dependency-free `benches/engine.rs` (`harness = false`, no criterion) for snapshot(2k)/diff/log-walk with coarse perf budgets that fail on order-of-magnitude regressions. `cargo bench` green within budget. |
| 2026-06-26 | Cleanup B DONE. Retired the `cli_*` compat surface from the engine crates by relocating it into the CLI. `sorrel-cli` `052ebde`: now a **lib+bin** owning `src/cli_policy.rs` + `src/cli_runner/` (workflow parse + local run + policy gate) + shared `CommandOutput`; integration tests consume the lib (`sorrel_cli::cli_policy`); dropped the `sorrel-runners` dependency (added `serde_yaml`); bumped core rev to `047b13b`. `sorrel-runners` `51e7c29`: removed `cli_runner` module and the `sorrel-core` dep — native runner model only. `sorrel-core` `047b13b`: removed `cli_policy` module + root re-exports — native `policy`/`authority` API only. All three: build/test/clippy/fmt clean. **Clean state reached: no mocks, engine-level exclusion, perf benches, no cross-crate compat duplication.** Next: Phase R (Hub push/pull as a Core object transport + ref store). |
...
| 2026-06-26 | Docs + Hub split pass. Answered "is Hub a server or web interface?": `sorrel-hub` is the JSON **API server** (no UI). Created a new **`sorrel-hub-web`** submodule (framework-free browser frontend that proxies to the Hub API; `npm test` 4/4) and added it to the root. Clarified roles everywhere (Hub README/AGENTS; root README/AGENTS): `sorrel-hub`=API, `sorrel-hub-web`=UI, `sorrel-web`=public marketing. Rewrote the root README (was a stub), fleshed out planned-module READMEs, and replaced every generic submodule AGENTS.md with module-specific guidance (stack, checks, Core boundary). Pushed docs to all submodule mains; advancing all root pointers (this also resolves the long-standing `sorrel-web` drift: `db12183` -> `6786303`, a clean forward move). |
| 2026-06-26 | P0-6/7/8/9 DONE — **P0 Quick Advance Goal complete**: `sorrel-cli` PR #9 (`81058ae`) adds `diff` (line-level unified hunks via dependency-free linediff module) and `log` (snapshot-DAG walk from HEAD; change create now links snapshot parents), plus diff/log tests and `DEMO.md`. Full demo `init->edit->status->change create->diff->log` works and persists across processes. json_output 24 + lib 8 pass; clippy + fmt clean. Root `sorrel-cli` pointer advanced. Remaining: Phase A engine hardening (perf: engine-level `.sorrel` exclusion + stat-cache, benches/CI budget), `cli_*` compat dedupe debt, then Git bridge / embedding surface / Hub / desktop / mobile per the plan. |
| 2026-06-26 | P0-4/5 DONE: `sorrel-cli` PR #8 (`16e6e14`) — real `change create -m` (stage tree minus `.sorrel`, snapshot, diff vs HEAD, record Change, advance HEAD; rejects empty/uninitialized) and real dirty `status` (added/modified/deleted vs HEAD). `repo.rs` ScratchDir + copy-excluding-`.sorrel` helpers. End-to-end demo (init->edit->status->change->status) persists. json_output 20 pass; clippy + fmt clean. Root `sorrel-cli` pointer advanced. Next: P0-6 line-diff, P0-7 log. |
| 2026-06-26 | P0-2/3 DONE: `sorrel-cli` PR #7 (`a40b34f`) makes `init`/`status` real and persistent — `init` opens a FileObjectStore at `.sorrel`, materializes an initial snapshot, writes real `manifest.json` + atomic `HEAD`; idempotent; `status` reads persisted state. New `src/repo.rs` helpers. Persistence-across-processes test added; clippy + fmt clean. Root `sorrel-cli` pointer advanced. (Task tool was hitting an infra error, so executed directly.) Next: P0-4/5 real `change create` + dirty `status`. |
