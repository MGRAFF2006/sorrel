...
| `sorrel-cli` | Done / merged | Real persistent `init`/`status`/`change create` (P0-2..5) on git-dep engine | PR #8 merged; root points to `sorrel-cli/main` at `16e6e14`; `init`+object store, `status` real dirty detection, `change create -m` records a real Change and advances HEAD; end-to-end demo persists; `cargo test --workspace` all pass, fmt + clippy clean. |
