...
Sorrel consumers. `sorrel-cli` currently embeds Core in `crates/sorrel-core`;
`tests/policy_conformance.rs` proves the embedded copy agrees with every
`expected` value. See `sorrel-protocol/docs/policy-conformance.md` for the
contract.
...
npm run export:conformance -- <path-to>/sorrel-cli/tests/conformance
```

(or run the root `scripts/sync-conformance.sh`), then re-run
