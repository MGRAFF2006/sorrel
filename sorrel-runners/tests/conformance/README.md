...
It is the language-neutral source of truth for the policy decisions the runner
gate must honor. `tests/policy_conformance.rs` proves the
`CorePermissionEvaluator` gate agrees with the manifest decisions and that
forged bundle-attached decisions cannot bypass Core evaluation. See
...
npm run export:conformance -- <path-to>/sorrel-runners/tests/conformance
```

(or run the root `scripts/sync-conformance.sh`), then re-run `cargo test`.
